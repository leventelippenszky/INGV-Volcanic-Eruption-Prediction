...
# XGBoost level 1 OOF predictions
oof_2 = pd.read_csv("oof_and_sub/oof_2.csv")
assert ~ (oof_1.segment_id != oof_2.segment_id).any()
...
# XGBoost level 1 SUB predictions
sub_2 = pd.read_csv("oof_and_sub/sub_2.csv")
assert ~ (sub_1.segment_id != sub_2.segment_id).any()
test_meta = pd.DataFrame({'segment_id': sub_1.segment_id,
                          'pred_1': sub_1.time_to_eruption,
                          'pred_2': sub_2.time_to_eruption})
...
# Save the SUB file
#submission.to_csv('sub_lgbm21.csv', header=True, index=False)
